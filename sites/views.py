...
def delete_log(request):
    ...
    try:
        log_data = LogData.get_all_logs(location_id, log_name)
        log_data = log_data[0]
...
def manage_log_update_info(request):
    ...
    if form.is_valid():
        ...
        try:
            ...
            logger_time_format_name = logger_time_format_data.get('logger_time_format_name')
        except IndexError:
            print("Index error!")
        ...
        try:
            log_file_info_exists = log_file_info_data[0]
            Log_file_info_by_log(log_id=log_id).update(
                log_file_path=log_file_path,
                log_file_line_num=log_file_line_num
            )
        except IndexError:
        ...
        try:
            log_update_info_exists = log_update_info_data[0]
            Log_update_schedule_by_log(log_id=log_id).delete()
        ...
        try:
            log_time_info_exists = log_time_info_data[0]
            Log_time_info_by_log(log_id=log_id).update(
                logger_time_format_id=logger_time_format_id,
                logger_time_format_name=logger_time_format_name,
